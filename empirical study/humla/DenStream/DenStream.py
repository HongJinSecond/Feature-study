...
class DenStream:
    ...
    def weights_to_points(self, data_points):
        ...
        """Hard pruning: if the X_org is within one micro-cluster (i.e., sim=1)
        set the distances of this X_org to other micro-clusters to zero. 
    ...
    def compute_CLs(self, X_norm, y_obv, nb_min_c1_refn=5):
        ...
        use_repo = False  # TODO 2021-11-25, 2022/8/19 to eliminate the related codes
        if use_repo:
        ...
        for cc in range(len(self.p_micro_clusters)):
            ...
            for _, ind in enumerate(inds):
                if y_obv[ind] == 0 and CLs_est[ind] < 1-self.theta_cl:
                    # There may be some X_org not belonging to any micro-cluster,
...
# X_org = np.random.random([1000, 5]) * 500
# clusterer = DenStream(lambd=0.1, eps=100, beta=0.5, mu=3)
# #for row in X_org[:100]:
...
# y = clusterer.fit_predict(X_org[100:])
