...
class OLD():
    ...
    def log(self, repo_id, Sample_nums):
        """
        ...
        print(f"-----Starting create structure-----")
        ...
        sizee=len(commitList[:Sample_nums])
        print('-----Starting feature extracttion-----')
        gc.enable()
        for commit in commitList[:Sample_nums]:
...
class BDFE():
    ...
    def log(self, repo_id, Sample_nums=None):
        """
        log(): Repository, Boolean -> Dictionary
        arguments: repo Repository: the repository to clone
            ingestion date
        ...
        print(f"-----Starting create structure BDFE-----")
        for data in info[:Sample_nums]:
            if data=="":
                continue
            c=CommitNode()
            d=data.split(',')
...
def offline_process(repo_id,Mode="New",Samples=None,Auto_Labeling = True):
    """
    ...
    if Mode=="New":
        ...
        #It takes time
        git=OLD()
        logging.info("Start Old model")
        
    commits = git.log(repo_id,Sample_nums=Samples)
