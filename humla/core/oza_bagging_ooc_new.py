...
class OzaBaggingClassifier_OOC_new(BaseSKMObject, ClassifierMixin, MetaEstimatorMixin):
    ...
    version can effectively handle data_stream streams.
    ...
    drawn by the binomial distribution. Since we can consider the data_stream stream
    ...
    # >>> from skmultiflow.data_stream import SEAGenerator
    ...
    def __init__(self, base_estimator=KNNADWINClassifier(), n_estimators=10, random_state=None,
        ...
        self.__configure()
        """Liyan, 2021-7-22 
        rho0: the size of class 0 (clean), corresponding to w- in Table 2 of Shuo paper.
        rho1: the size of class 1 (defect-inducing), corresponding to w+ in Table 2 of Shuo paper.
    ...
    def partial_fit(self, X, y, CLs_np, classes=None, sample_weight=None):
        ...
        2021-12-14  Shuxian finds a bug making nb_tree useless as all trees employ exactly the same training data_stream
        """
        if self.classes is None:
            if classes is None:
                raise ValueError("The first partial_fit call should pass all the classes.")
        ...
        r, _ = get_dimensions(X)  # r: data_stream size
        y = y.reshape(r, )
        CLs_np = CLs_np.reshape(r, )

        for j in range(r):  # for each data_stream point
    ...
    def predict(self, X):
        """ Predict classes for the passed data_stream.

        Parameters
        ----------
        X : numpy.ndarray of shape (n_samples, n_features)
            The set of data_stream XX to predict the class yy for.
