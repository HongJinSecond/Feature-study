...
from humla.DenStream.MicroCluster import MicroCluster
...
class DenStream_new:
    ...
    def __init__(self, theta_cl=0.8, lambd=1, eps=1, beta=2, mu=2):
        ...
        lambd: float, optional
            ...
            importance of the historical data_stream compared to more recent data_stream.
        eps : float, optional
        ...
        self.beta = beta
        self.mu = mu
        self.t = 0  # 2021-11-24 TODO # of used data_stream?
        ...
        """Liyan: entire data_stream info (all micro-clusters)"""
        self.nb_all_clean = 0
    ...
    def partial_fit(self, X, y, sample_weight=None):
        """
        Liyan Song
        2021-12-6   the label value of unlabelled data_stream is auto tackled here, no worries.

        ================================================
        Online learning.

        XX : {array-like, sparse matrix}, shape (n_samples, n_features)
            Subset of training data_stream
        y : Ignored
        ...
        # "data_stream %d." % (n_features, self.coef_.shape[-1]))
    ...
    def predict(self, X):
        ...
        In DenStream, it happens that a data_stream does not belong to any micro-cluster.
    ...
    def micro_clusters_bias(self):
        ...
        opt_ = 0
        if opt_ == 0:  # only labelled data_stream
            nb_all_data = self.nb_all_clean + self.nb_all_defect
        elif opt_ == 1:  # all data_stream
    ...
    def weights_to_points(self, data_points):
        ...
        """1. derive similarity and concept-drift betw each data_stream and micro-cluster"""
        ...
        """Hard pruning: if the data_stream is within one micro-cluster (i.e., sim=1)
        set the distances of this data_stream to other micro-clusters to zero. 
        ...
        """2. primary micro-cluster weights across all data_stream points"""
        ...
        each data_stream has its own threshold, determine heuristically"""
        for dd in range(nb_data):
    ...
    class repo_c1_data:
        """
        The data_stream repository for the latest defect-inducing data_stream.
        ...
        def insert_c1_data(self, new_X_norm, new_y_obv, new_y_time):
            ...
            """
            This method selects the class 1 data_stream and insert them to the class-1 data_stream repository.
            ...
            # retain the latest class-1 data_stream if exceed max_size
        ...
        def pop_def_data(self, nb_pop):
            if nb_pop > self.get_data_num():
                print("\tAttention: # required pop def data_stream > the repository size. Pop out all we have.")
    ...
    def check_data_dim(self, X_norm, y_obv):
        if X_norm.ndim == 2:
            assert X_norm.shape[0] == y_obv.shape[0], "the #data_stream NOT match #label"
        elif X_norm.ndim == 1:
            assert np.atleast_1d(y_obv).shape[0] == 0, "the #data_stream NOT match #label"

    def compute_CLs(self, X_norm, y_obv, classifier, rnd_seed, k_power=2):
        """ the main CL framework:
        Compute CLs of observed yy of training data_stream.
        ...
        The class-1 refinement technique is quite aggressive towards class-1 data_stream.

        All actual class-1 data_stream can be correctly identified and fully used in the training process.
        Moreover, noisy class-1 data_stream might also be detected and properly used in the training process,
        for which the class-1 confidence values are high.
        However, some class-0 data_stream might be mis-recognized as class-1, and filtered out of the training process.

        If we do not adopt the refinement mechanism, our derived CLs usually fail in recognizing
        noisy class-0 labeled data_stream. Considering the fact that class 0 is typically the majority,
        ...
        def primary_CLs(y_obv, bias_c0_step1_2, bias_c1_step1_2, weight_data_2_cluster):
            ...
            """
            my_tolerance = 0.00001  # threshold if a data_stream point belongs to any p-cluster
            nb_data = len(y_obv)
            ...
            for dd, y_1obv in enumerate(y_obv):
                ...
                if np.sum(clu_weights_data) >= my_tolerance:
                    ...
                    """[2021-11-25] No micro-cluster wants this data_stream.
        ...
        """step 4. class 1 (0) label confidence of all data_stream:
        The issue that a data_stream does not belong to any micro-cluster is handled here"""
        data_cl_c0, data_cl_c1, use_cluster = primary_CLs(y_obv, cluster_rep_c0, cluster_rep_c1, weights_data2cluster)

        """step 5. refine class-1 confidence of all data_stream"""
        ...
        if use_repo and self.repo_c1_data.get_data_num() and nb_repo_request:
            ...
            # make sure that repo data_stream in use should have "repo_use_cluster = 1"
            ...
            """select among repo data_stream points"""
            if nb_repo_request < nb_repo_pop:
                '''type-1. select correctly predicted repo data_stream'''
                ...
                '''type-2. select repo data_stream from the same or diff clusters depending on online imbalance ratio'''
                ...
                if classifier.rho1 <= classifier.rho0:
                    ...
                    repo_id_clu = np.where(cluster_id_repo != cluster_id_train)[0]
                # debug: if no repo data_stream can meet the repository criteria, type-2 is not adopted
        ...
        """step 5-2. refine c1 confidence based on the c1 training data_stream and c1 data_stream repository.
        The goal is to set the c1 confidence of the c1 labeled data_stream equaling to the maxima 1."""
        ...
        2022/4/15 the below setup is reasonable if
            1) the number of repo data_stream is not *small*, and
        ...
        if len(c1_data_cl_c1) >= 1:
            if len(c1_data_cl_c1) > 1:
                ...
                # being the bottom CL of all c-1 data_stream acc to the 68-95-99 rule of Gaussian pdf.
            ...
            id_potential_c1 = np.where(data_cl_c1_refine >= btm_cl_c1)[0]  # potential c1 data_stream
            if min_cl_c1 > 0:
        ...
        if option_ == 0:  # original, no CL refinement
            ...
            cl_y_obv[np.where(y_obv == 1)[0]] = 1  # force CLs of defect data_stream to the maxima
    ...
    def revise_cluster_info(self, X_norm, y_obv, data_time, CLs_est):
        """ Update data_stream number of micro-clusters acc the derived label CLs.
        ...
        We need a threshold above which we should revise the data_stream info of micro-clusters,
        ...
        # 2022/4/12 todo remove the below data_stream info revision
        ...
        for cc in range(len(self.p_micro_clusters)):
            ...
            for _, ind in enumerate(inds):
                if y_obv[ind] == 0 and CLs_est[ind] < 1-self.theta_cl:
                    # There may be some data_stream not belonging to any micro-cluster,
                    ...
                    # whole data_stream info update: sometimes it helps (broadleaf, noise=0.3),
    ...
    def plot_cluster(self, X_norm, y, pca, title_info=None, x_lim=None, y_lim=None, plot_clu=True):
        """
        plot in 2D the real jit-sdp data_stream and the online clustering process.
        ...
        """
        # plot all data_stream points with the true yy
...
# data_stream = np.random.random([1000, 5]) * 500
# clusterer = DenStream(lambd=0.1, eps=100, beta=0.5, mu=3)
# #for row in data_stream[:100]:
...
# y = clusterer.fit_predict(data_stream[100:])
